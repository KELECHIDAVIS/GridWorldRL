...
export function GridCell({ data, mode, onClick }: GridCellProps) {
  ...
  if (data.type === 'wall') {
    return <div className={`${base} bg-zinc-800`} onClick={onClick} />
  }

  if (data.type === 'start') {
    return (
      <div className={`${base} bg-zinc-900 border border-teal-700 text-teal-400 font-bold text-[10px]`} onClick={onClick}>
        S
      </div>
    )
  }
  ...
  if (data.type === 'goal') {
    return (
      <div className={`${base} bg-zinc-900 border border-green-700 text-green-400 font-bold text-[10px]`} onClick={onClick}>
        G
      </div>
    )
  }
  ...
  if (data.type === 'agent') {
    return (
      <div
        ...
        style={{ borderColor: data.agentColor, background: `${data.agentColor}22` }}
        onClick={onClick}
      />
    )
  }
  ...
  if (mode === 'value' && data.value !== undefined) {
    return (
      <div
        ...
        style={{ background: valueToColor(data.value), color: '#c2c0b6' }}
        onClick={onClick}
      >
  ...
  return (
    <div className={`${base} bg-zinc-900 border border-zinc-800/50`} onClick={onClick} />
  )
}